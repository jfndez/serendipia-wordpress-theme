...
function pinbin_menu_options() {
	//add_theme_page( $page_title, $menu_title, $capability, $menu_slug, $function);
     add_theme_page('Pinbin Options', 'Opciones', 'edit_theme_options', 'pinbin-settings', 'pinbin_admin_options_page');
}
...
function pinbin_admin_options_page() {
	?>
		...
		<div class="wrap">
			...
			<h2><?php _e( 'Opciones del tema', 'pinbin' ); ?></h2>
			...
			<form id="form-pinbin-options" action="options.php" method="post" enctype="multipart/form-data">
				...
				<p class="submit">
					<?php submit_button( __( 'Guardar cambios', 'pinbin' ), 'primary', 'theme_pinbin_options[submit]', false ); ?>
					<?php submit_button( __( 'Restaurar a la configuración original', 'pinbin' ), 'secondary', 'theme_pinbin_options[reset]', false ); ?>
				</p>
			 <div class="credit"><?php _e( 'Adaptación de <a href="http://www-jose-fernandez.com.es">José Fernández</a> del tema Pinbin', 'pinbin' ); ?> <a href="http://colorawesomeness.com/themes/pinbin/">Color Awesomeness</a>.</div>
...
function pinbin_options_settings_init() {
	...
	add_settings_section('pinbin_settings_header', __( 'Opciones para el logo', 'pinbin' ), 'pinbin_settings_header_text', 'pinbin');
	...
	add_settings_field('pinbin_setting_logo_preview',  __( 'Previo', 'pinbin' ), 'pinbin_setting_logo_preview', 'pinbin', 'pinbin_settings_header');
}
...
function pinbin_settings_header_text() {
	?>
		<p><?php _e( 'Sube tu logo. El tema soporta GIF, JPEG, PNG.', 'pinbin' ); ?></p>
	<?php
}
...
function pinbin_setting_logo() {
	...
	?>
		...
		<input id="upload_logo_button" type="button" class="button" value="<?php _e( 'Subir logo', 'pinbin' ); ?>" />
		<?php if ( '' != $pinbin_options['logo'] ): ?>
			<input id="delete_logo_button" name="theme_pinbin_options[delete_logo]" type="submit" class="button" value="<?php _e( 'Eliminar logo', 'pinbin' ); ?>" />
		<?php endif; ?>
		<span class="description"><?php _e('Subir un logo para la cabecera de la web.', 'pinbin' ); ?></span>
	<?php
}
